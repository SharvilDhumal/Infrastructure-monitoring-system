...
const StatCard = ({ label, count, icon: Icon, color, delay }) => {
  return (
    <motion.div
      ...
      className="flex-1 w-full p-4 sm:p-6 md:p-8 rounded-2xl bg-white border-4 border-[#002147] shadow-[2px_2px_0px_#002147] sm:shadow-[4px_4px_0px_#002147] hover:translate-y-1 hover:translate-x-1 hover:shadow-none transition-all duration-200"
    >
      ...
      <div className="flex flex-col items-center justify-center mb-2 md:mb-4">
        <motion.span
          initial={{ scale: 0.5 }}
          animate={{ scale: 1 }}
          className="text-2xl sm:text-3xl md:text-4xl font-black text-[#002147] tracking-tight mb-1 md:mb-2"
        >
          {count}
        </motion.span>
        <span className="text-xs sm:text-sm md:text-lg font-bold text-gray-800 uppercase tracking-wide text-center">{label}</span>
      </div>
    </motion.div>
  );
};
...
const DashboardStats = ({ stats }) => {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-6 mb-8 w-full">
